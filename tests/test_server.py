...
class TestWhatTheToolsPromise:
    ...
    A tool that declares it may be replayed automatically after the frontend
    repairs auth, with no second confirmation from anyone. The messaging reads
    therefore claim this only while their passive response capture preserves
    conversation state.
    """

    async def test_passive_messaging_reads_claim_read_only(self):
        mcp = create_mcp_server()
        passive_reads = {"get_inbox", "get_conversation", "search_conversations"}
        claiming = set()
        for name in passive_reads:
