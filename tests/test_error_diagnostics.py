...
def test_build_issue_diagnostics_includes_existing_issues(monkeypatch, tmp_path):
    ...
    assert REPORT_GUIDANCE in issue_body
...
def test_format_tool_error_empty_search_still_requires_issue_search():
    ...
    assert "Search open and closed issues yourself before filing" in message
    assert REPORT_GUIDANCE in message


def test_format_tool_error_ignores_legacy_gist_command():
    """A payload written by an older server still renders no upload command."""
    message = format_tool_error_with_diagnostics(
        "Scrape failed",
        {
            "issue_template_path": "/tmp/issue.md",
            ...
            "runtime": {
                ...
                "trace_dir": "/tmp/trace",
                "suggested_gist_command": 'gh gist create "/tmp/issue.md" "/tmp/trace/trace.jsonl"',
            },
        },
    )

    assert "gh gist create" not in message
    assert "gist" not in message.lower()
    assert "- Local trace artifacts: /tmp/trace" in message
...
async def test_build_issue_diagnostics_skips_network_search_in_event_loop(
    ...
    assert "Search open and closed issues yourself before filing" in issue_body
