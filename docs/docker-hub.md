...
- **Messaging**: List the inbox, read a conversation by username or thread ID, and search messages without changing read/unread state; compose/send a new message with explicit confirmation (profile-based send may open a separate DM rather than reply in an existing thread)
...
| `TOOL_CALL_GAP_SECONDS` | `8` | Read minimum of the random gap between two tool calls, in seconds; the default band is 8–20 s for reads and 20–60 s for `connect_with_person` / `send_message`, and every bound scales with this value (`0` = no spacing). LinkedIn answers bursts of navigations with HTTP 429 and checkpoints. Tools answered from local disk never wait. |
| `HOURLY_ACTIONS_MAX` | `40` | LinkedIn-touching actions allowed in any rolling hour across every session sharing the ledger; a call over it is refused with an `account_cooldown` error naming `resume_at`; the bulk enrichment tools plan their bunch against the headroom instead. |
| `ACCOUNT_COOLDOWN_DISABLED` | unset | `1` stops a throttle signal (HTTP 429, checkpoint, rate-limit page) from pausing the account. By default one signal pauses every session for 30 min, escalating to 2 h, 4 h and 8 h for further signals within a day; delete `until` from `jobs/__account_cooldown__.json` to release it by hand. |
