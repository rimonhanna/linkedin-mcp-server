...
| `search_jobs` | Search for jobs with keywords and location filters. Returns `job_ids` plus `total` (`{count, exact}`, the result count LinkedIn advertises; `exact` is false for a lower bound such as "500+") and `promoted_job_ids` (the subset of `job_ids` shown as promoted; present only when every page could be read), alongside the raw text |
