...
REPORT_GUIDANCE = (
    "Reporting: these notes are local evidence, not a report. Trace runs and "
    "server logs hold scraped LinkedIn data and stay on this machine. To file "
    f"a bug, fill in the issue form ({ISSUE_URL}) from what you know, paste "
    "only redacted excerpts, search open and closed issues first, then show "
    "the final issue or comment to the human and ask for an explicit yes "
    "before posting."
)
...
def format_tool_error_with_diagnostics(
    ...
    """Append local note locations and reporting guidance to a tool-facing error."""
    ...
    if existing_issues:
        ...
        lines.append(
            "- Advisory open-issue search was skipped in async server context. Search open and closed issues yourself before filing."
        )
    else:
        lines.append(
            "- Diagnostics did not establish a canonical match. Search open and closed issues yourself before filing."
        )
    return "\n".join(lines)


def _render_issue_template(payload: dict[str, Any]) -> str:
    # Local notes cannot be a finished report: the reporter supplies the
    ...
    return (
        "\n".join(
            [
                ...
                "",
                "These notes are local troubleshooting aids. They are not a finished report.",
                REPORT_GUIDANCE,
                "",
                "## Captured failure",
